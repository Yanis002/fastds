...
class NARC:
    ...
    def _initFromData(self, data: bytes) -> None:
        ...
        magic, bom, version, filesize, headersize, numblocks = _common.NDS_STD_FILE_HEADER.unpack_from(data, 0)
        ...
        if magic != b"NARC":
            raise ValueError(f"Wrong magic (should be b'NARC', instead found {magic})")
    ...
    def save(self) -> bytes:
        ...
        fatbData = bytearray()
        fatbData.extend(struct.pack("<4sII", b"FATB"[::-1], 0x0C + 8 * len(self.files), len(self.files)))
        ...
        _common.NDS_STD_FILE_HEADER.pack_into(data, 0, b"NARC", bom, version, len(data), 0x10, 3)
        return bytes(data)
    ...
    def __str__(self):
        ...
        if notes:
            notes.insert(0, "")  # so it'll begin with a space
        notes = " ".join(notes)
