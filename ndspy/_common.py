...
def loadNullTerminatedStringFrom(data, offset, charWidth=1, encoding="latin-1"):
    """
...
def loadInfoBlock(data, off, expectedEntryLengths=None):
    ...
    unkHeaderLen, unkLen, unkUnk04 = struct.unpack_from("<HHI", data, off)
    off += 8
    ...
    eachEntryLen, entriesLen = struct.unpack_from("<HH", data, off)
...
def saveInfoBlock(entries, entryLengths):
    ...
    for e in entries:
        if len(e[3]) != entryLengths:
            raise ValueError(
                f"One of the entries is not of length {entryLengths} like it should be (actual length is {len(e[3])})"
            )
        data.extend(e[3])
...
def ensurePIL():
    ...
    if not HavePIL:
        raise RuntimeError(
            'PIL (Pillow) is not installed, so ndspy cannot render image data as PIL images. Run "pip install Pillow" to install it.'
        )
