...
class Folder:
    ...
    def __getitem__(self, key: int | str) -> Folder | str | int | NoReturn:
        ...
        if isinstance(key, int):
            ...
            raise TypeError(f'Folders can only convert between strings and ints, not "{type(key)}".')
    ...
    def __repr__(self):
        return f"{type(self).__name__}({self.folders!r}, {self.files!r}, {self.firstID!r})"
...
def save(root: Folder) -> bytes:
    ...
    def parseFolder(d: Folder, parentID: int) -> int:
        ...
        for file in d.files:
            ...
            # folder.
            if len(file) > 127:
                raise ValueError(f'Filename "{file}" is {len(file)} characters long (maximum is 127)!')
        ...
        for folderName, folder in d.folders:
            ...
            if len(folderName) > 127:
                raise ValueError(f'Folder name "{folderName}" is {len(folderName)} characters long (maximum is 127)!')
