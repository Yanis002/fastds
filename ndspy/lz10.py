...
def main(args: Sequence[str] | None = None) -> None:
    ...
    """
    parser = argparse.ArgumentParser(description="ndspy.lz10 CLI: Compress or decompress files using LZ10.")
    subparsers = parser.add_subparsers(title="commands", description="(run a command with -h for additional help)")
    ...
    parser_compress = subparsers.add_parser("compress", aliases=["c"], help="compress a file")
    parser_compress.add_argument("input_file", type=pathlib.Path, help="input file to compress")
    parser_compress.add_argument("output_file", nargs="?", type=pathlib.Path, help="what to save the compressed file as")
    ...
    parser_decompress = subparsers.add_parser("decompress", aliases=["d"], help="decompress a file")
    parser_decompress.add_argument("input_file", type=pathlib.Path, help="input file to decompress")
    parser_decompress.add_argument("output_file", nargs="?", type=pathlib.Path, help="what to save the decompressed file as")
