...
class SingletonUpdater:
    ...
    def merge_requests(self):
        if len(self._merge_requests) == 0:
            return {}
        return {mr["id"]: mr["title"] for mr in self._merge_requests}
    ...
    def get_tags(self):
        ...
        if self._tags is None:
            ...
            self.print_verbose("{} branch found, no releases: {}".format(branch, self._tags[0]))
            ...
            if not self._include_branches:
                ...
                self.print_verbose("Most recent tag found:" + str(self._tags[n]["name"]))
    ...
    def create_backup(self):
        ...
        if os.path.isdir(local):
            try:
                ...
                self.print_verbose("Failed to removed previous backup folder, continuing")
                self.print_trace()
        ...
        if clean:
            try:
                ...
                for f in files:
                    ...
                    self.print_verbose("Clean removing file {}".format(os.path.join(base, f)))
                for f in folders:
                    if os.path.join(base, f) is self._updater_path:
                        continue
                    shutil.rmtree(os.path.join(base, f))
                    self.print_verbose("Clean removing folder and contents {}".format(os.path.join(base, f)))
            ...
            for file in files:
                ...
                if os.path.isfile(dest_file):
                    ...
                    if replaced:
                        ...
                        self.print_verbose("Pattern not matched to {}, not overwritten".format(os.path.basename(dest_file)))
                else:
    ...
    def get_json_path(self):
        ...
        """
        json_path = os.path.join(self._updater_path, "{}_status.json".format(self._addon_package))
        old_json_path = os.path.join(self._updater_path, "_status.json")
...
class BitbucketEngine:
    ...
    def form_repo_url(self, updater):
        return "{}/2.0/repositories/{}/{}".format(self.api_url, updater.user, updater.repo)
    ...
    def get_zip_url(self, name, updater):
        return "https://bitbucket.org/{user}/{repo}/get/{name}.zip".format(user=updater.user, repo=updater.repo, name=name)
...
class GitlabEngine:
    ...
    def form_branch_url(self, branch, updater):
        ...
        return "{}/repository/archive.zip?sha={}".format(self.form_repo_url(updater), branch)

    def form_mr_url(self, mr, updater):
        raise NotImplementedError

    def get_zip_url(self, sha, updater):
        return "{base}/repository/archive.zip?sha={sha}".format(base=self.form_repo_url(updater), sha=sha)
